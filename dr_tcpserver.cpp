...
CTcpServer::CTcpServer(QObject *pObject)
    : QObject(pObject)
{
...
{
    if (BAPackageMark.size() == PACKAGE_MARK_SIZE)
        m_BAPackageMark = BAPackageMark;
}
...
void CTcpServer::socketReadyRead()
{
    ...
    do {
        ...
        if (m_hashPackageSize.value(pSocket, 0) == 0) {
            if (pSocket->bytesAvailable() < (int) (sizeof(quint16) + PACKAGE_MARK_SIZE))
                return;
        ...
        if (pSocket->bytesAvailable() < m_hashPackageSize.value(pSocket))
            return;
