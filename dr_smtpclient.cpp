...
CSmtpClient::CSmtpClient(QObject *pObject)
    : QObject(pObject)
{
    m_pSocket = 0;
...
bool CSmtpClient::connectToHost()
{
    ...
    if (!sendMessage("EHLO " + m_sName.toUtf8()))
        return false;
    if (!waitForResponse())
        return false;
...
bool CSmtpClient::login(const QString &sUser, const QString &sPassword, AuthMethod iMethod)
{
    ...
    if (iMethod == AuthPlain) {
        ...
        if (!sendMessage(sUser.toUtf8().toBase64()))
            return false;
        if (!waitForResponse())
            return false;
        if (m_iServerReply != 334) {
        ...
        if (!sendMessage(sPassword.toUtf8().toBase64()))
            return false;
        if (!waitForResponse())
            return false;
        if (m_iServerReply != 235) {
                           ...
                           const QString &sBody)
{
    ...
    if (!sendMessage("MAIL FROM:<" + sFrom.toUtf8() + ">"))
        return false;
    if (!waitForResponse())
        return false;
    if (m_iServerReply != 250) {
    ...
    if (!sendMessage("RCPT TO:<" + sTo.toUtf8() + ">"))
        return false;
    if (!waitForResponse())
        return false;
    if (m_iServerReply != 250) {
    ...
    QByteArray BA = QByteArray("From: ")
                        .append(sNameFrom.toUtf8())
                        .append(" <")
                        .append(sFrom)
                        .append(">\r\n")
                        .append("To: ")
                        .append(sNameTo.toUtf8())
                        .append(" <")
                        .append(sTo)
                        .append(">\r\n")
                        .append("Subject: ")
