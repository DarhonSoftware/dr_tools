...
CPop3Client::CPop3Client(QObject *pObject) : QObject(pObject)
{
  m_pSocket=0;
...
bool CPop3Client::connectToHost()
{
  ...
  if (m_iConnectionType == TlsConnection)
  {
    ...
    if (sendMessage("STLS")) return false;
    if (!waitForResponse(false)) return false;
    if (m_lstServerReply.first().left(3)!="+OK")
    {
...
bool CPop3Client::login(const QString &sUser, const QString &sPassword, AuthMethod iMethod)
{
  ...
  if (iMethod == AuthAPOP)
  {
    ...
    if (!sendMessage("APOP " + sUser.toUtf8() + " " + QCryptographicHash::hash(m_BATimestamp+m_sSecret.toUtf8(),QCryptographicHash::Md5))) return false;
    if (!waitForResponse(false)) return false;
    if (m_lstServerReply.first().left(3)!="+OK")
    {
  ...
  else if (iMethod == AuthPlain)
  {
    ...
    if (!sendMessage("USER " + sUser.toUtf8())) return false;
    if (!waitForResponse(false)) return false;
    if (m_lstServerReply.first().left(3)!="+OK")
    {
    ...
    if (!sendMessage("PASS " + sPassword.toUtf8())) return false;
    if (!waitForResponse(false)) return false;
    if (m_lstServerReply.first().left(3)!="+OK")
    {
...
bool CPop3Client::readMail()
{
  ...
  if (!sendMessage("STAT")) return false;
  if (!waitForResponse(false)) return false;
  if (m_lstServerReply.first().left(3)!="+OK")
  {
  ...
  {
    if (!sendMessage("RETR " + QByteArray::number(i+1))) return false;
  ...
  for (int i=0;i<iMails;i++)
  {
    if (!sendMessage("DELE " + QByteArray::number(i+1))) return false;
    if (!waitForResponse(false)) return false;
    if (m_lstServerReply.first().left(3)!="+OK")
    {
...
bool CPop3Client::waitForResponse(bool bMultiline)
{
  ...
  do
  {
    ...
    while (m_pSocket->canReadLine())
    {
      ...
      if (!bMultiline) return true;
      if (BAResponse.left(4)=="-ERR") return true;
    }
  } while (true);
}
