...
CParticleClient::CParticleClient(QObject *pObject)
    : QObject(pObject)
{
...
{
    if (sToken.size() == TOKEN_LENGTH)
        m_sAccessToken = sToken;
}
...
bool CParticleClient::listDevices()
{
    ...
    m_EventLoop.exec();
    if ((!m_sLastError.isEmpty()) && (m_sLastError != tr(ERROR_RUNNING)))
        return false;
